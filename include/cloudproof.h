 ...
 */
typedef int (*FetchEntryTableCallback)(unsigned char *entries_ptr, unsigned int *entries_len,
                                       const unsigned char *uids_ptr, unsigned int uids_len);
 ...
 */
typedef int (*FetchChainTableCallback)(unsigned char *chains_ptr, unsigned int *chains_len,
                                       const unsigned char *uids_ptr, unsigned int uids_len);

/**
 * See
 * [`FindexCallbacks::upsert_entry_table()`](cosmian_findex::FindexCallbacks::upsert_entry_table).
 *
 * # Serialization
 *
 * The input is serialized as follows:
 ...
 * `LEB128(n_lines) || UID_1 || LEB128(value_1.len()) || value_1 || ...`
 */
typedef int (*UpsertEntryTableCallback)(unsigned char *outputs_ptr, unsigned int *outputs_len,
                                        const unsigned char *entries_ptr, unsigned int entries_len);

/**
 * See
 * [`FindexCallbacks::insert_chain_table()`](cosmian_findex::FindexCallbacks::insert_chain_table).
 *
 * # Serialization
 *
 * The input is serialized as follows:
...
/**
 * See
 * [`FindexCallbacks::fetch_all_entry_table_uids()`](cosmian_findex::FindexCallbacks::fetch_all_entry_table_uids).
 *
 * The output should be deserialized as follows:
 *
 * `UID_1 || UID_2 || ... || UID_n`
...
int h_aes256gcm_encrypt(unsigned char *output_ptr, int *output_len, const char *plaintext_ptr,
                        int plaintext_len, const char *key_ptr, int key_len, const char *nonce_ptr,
                        int nonce_len, const char *authenticated_data_ptr,
                        int authenticated_data_len);

int h_aes256gcm_decrypt(unsigned char *output_ptr, int *output_len, const char *ciphertext_ptr,
                        int ciphertext_len, const char *key_ptr, int key_len, const char *nonce_ptr,
                        int nonce_len, const char *authenticated_data_ptr,
                        int authenticated_data_len);
 ...
 * # Safety
 */
int h_add_policy_axis(char *updated_policy_ptr, int *updated_policy_len,
                      const char *current_policy_ptr, int current_policy_len, const char *axis_ptr);

/**
 * # Safety
 */
int h_rotate_attribute(char *updated_policy_ptr, int *updated_policy_len,
                       const char *current_policy_ptr, int current_policy_len,
 ...
 * # Safety
 */
int h_generate_master_keys(char *msk_ptr, int *msk_len, char *mpk_ptr, int *mpk_len,
                           const char *policy_ptr, int policy_len);
 ...
 * # Safety
 */
int h_generate_user_secret_key(char *usk_ptr, int *usk_len, const char *msk_ptr, int msk_len,
                               const char *user_policy_ptr, const char *policy_ptr, int policy_len);
 ...
 * # Safety
 */
int h_update_master_keys(char *updated_msk_ptr, int *updated_msk_len, char *updated_mpk_ptr,
                         int *updated_mpk_len, const char *current_msk_ptr, int current_msk_len,
                         const char *current_mpk_ptr, int current_mpk_len, const char *policy_ptr,
                         int policy_len);
 ...
 * # Safety
 */
int h_refresh_user_secret_key(char *updated_usk_ptr, int *updated_usk_len, const char *msk_ptr,
                              int msk_len, const char *current_usk_ptr, int current_usk_len,
                              const char *user_policy_ptr, const char *policy_ptr, int policy_len,
 ...
 * # Safety
 */
int32_t h_create_encryption_cache(int *cache_handle, const char *policy_ptr, int policy_len,
                                  const char *mpk_ptr, int mpk_len);
 ...
 * # Safety
 */
int h_encrypt_header_using_cache(char *symmetric_key_ptr, int *symmetric_key_len,
                                 char *header_bytes_ptr, int *header_bytes_len, int cache_handle,
                                 const char *encryption_policy_ptr, const char *header_metadata_ptr,
                                 int header_metadata_len, const char *authentication_data_ptr,
 ...
 * # Safety
 */
int h_encrypt_header(char *symmetric_key_ptr, int *symmetric_key_len, char *header_bytes_ptr,
                     int *header_bytes_len, const char *policy_ptr, int policy_len,
                     const char *mpk_ptr, int mpk_len, const char *encryption_policy_ptr,
                     const char *header_metadata_ptr, int header_metadata_len,
                     const char *authentication_data_ptr, int authentication_data_len);
 ...
 * # Safety
 */
int h_decrypt_header_using_cache(char *symmetric_key_ptr, int *symmetric_key_len,
                                 char *header_metadata_ptr, int *header_metadata_len,
                                 const char *encrypted_header_ptr, int encrypted_header_len,
                                 const char *authentication_data_ptr, int authentication_data_len,
                                 int cache_handle);
 ...
 * # Safety
 */
int h_decrypt_header(char *symmetric_key_ptr, int *symmetric_key_len, char *header_metadata_ptr,
                     int *header_metadata_len, const char *encrypted_header_ptr,
                     int encrypted_header_len, const char *authentication_data_ptr,
                     int authentication_data_len, const char *usk_ptr, int usk_len);
 ...
 * # Safety
 */
int h_dem_encrypt(char *ciphertext_ptr, int *ciphertext_len, const char *symmetric_key_ptr,
                  int symmetric_key_len, const char *authentication_data_ptr,
                  int authentication_data_len, const char *plaintext_ptr, int plaintext_len);

/**
 *
 * # Safety
 */
int h_dem_decrypt(char *plaintext_ptr, int *plaintext_len, const char *symmetric_key_ptr,
                  int symmetric_key_len, const char *authentication_data_ptr,
                  int authentication_data_len, const char *ciphertext_ptr, int ciphertext_len);
 ...
 * # Safety
 */
int h_hybrid_encrypt(char *ciphertext_ptr, int *ciphertext_len, const char *policy_ptr,
                     int policy_len, const char *mpk_ptr, int mpk_len,
                     const char *encryption_policy_ptr, const char *plaintext_ptr,
                     int plaintext_len, const char *header_metadata_ptr, int header_metadata_len,
                     const char *authentication_data_ptr, int authentication_data_len);
 ...
 * # Safety
 */
int h_hybrid_decrypt(char *plaintext_ptr, int *plaintext_len, char *header_metadata_ptr,
                     int *header_metadata_len, const char *ciphertext_ptr, int ciphertext_len,
                     const char *authentication_data_ptr, int authentication_data_len,
                     const char *usk_ptr, int usk_len);
...
int h_ecies_x25519_generate_key_pair(unsigned char *public_key_ptr, int *public_key_len,
                                     unsigned char *private_key_ptr, int *private_key_len);

int h_ecies_salsa_seal_box_encrypt(unsigned char *output_ptr, int *output_len,
                                   const char *plaintext_ptr, int plaintext_len,
                                   const char *public_key_ptr, int public_key_len,
                                   const char *authentication_data_ptr,
                                   int authentication_data_len);

unsigned int h_ecies_salsa_seal_box_get_encryption_overhead(void);

int h_ecies_salsa_seal_box_decrypt(unsigned char *output_ptr, int *output_len,
                                   const char *ciphertext_ptr, int ciphertext_len,
                                   const char *private_key_ptr, int private_key_len,
                                   const char *authentication_data_ptr,
                                   int authentication_data_len);
 ...
 */
int h_search(char *search_results_ptr, int *search_results_len, const char *master_key_ptr,
             int master_key_len, const uint8_t *label_ptr, int label_len, const char *keywords_ptr,
             unsigned int entry_table_number, ProgressCallback progress_callback,
 ...
 * Cannot be safe since using FFI.
 */
int h_upsert(const uint8_t *master_key_ptr, int master_key_len, const uint8_t *label_ptr,
             int label_len, const char *additions_ptr, const char *deletions_ptr,
             unsigned int entry_table_number, FetchEntryTableCallback fetch_entry,
             UpsertEntryTableCallback upsert_entry, InsertChainTableCallback insert_chain);
 ...
 * Cannot be safe since using FFI.
 */
int h_live_compact(const uint8_t *master_key_ptr, int master_key_len,
                   int num_reindexing_before_full_set, unsigned int entry_table_number,
                   FetchAllEntryTableUidsCallback fetch_all_entry_table_uids,
                   FetchEntryTableCallback fetch_entry, FetchChainTableCallback fetch_chain,
 ...
 * Cannot be safe since using FFI.
 */
int h_compact(const uint8_t *old_master_key_ptr, int old_master_key_len,
              const uint8_t *new_master_key_ptr, int new_master_key_len,
              const uint8_t *new_label_ptr, int new_label_len, int num_reindexing_before_full_set,
 ...
 * Cannot be safe since using FFI.
 */
int h_search_cloud(char *search_results_ptr, int *search_results_len, const char *token_ptr,
                   const uint8_t *label_ptr, int label_len, const char *keywords_ptr,
 ...
 * Cannot be safe since using FFI.
 */
int h_upsert_cloud(const char *token_ptr, const uint8_t *label_ptr, int label_len,
                   const char *additions_ptr, const char *deletions_ptr, const char *base_url_ptr);
#endif
 ...
 * Cannot be safe since using FFI.
 */
int h_generate_new_token(uint8_t *token_ptr, int *token_len, const char *index_id_ptr,
                         const uint8_t *fetch_entries_seed_ptr, int fetch_entries_seed_len,
                         const uint8_t *fetch_chains_seed_ptr, int fetch_chains_seed_len,
                         const uint8_t *upsert_entries_seed_ptr, int upsert_entries_seed_len,
                         const uint8_t *insert_chains_seed_ptr, int insert_chains_seed_len);
#endif
 ...
 */
int h_fpe_encrypt_alphabet(unsigned char *output_ptr, int *output_len, const char *alphabet_id_ptr,
                           const char *input_ptr, const char *key_ptr, int key_len,
                           const char *tweak_ptr, int tweak_len,
 ...
 * `0` means success, while a non-zero value represents an error code.
 */
int h_fpe_decrypt_alphabet(unsigned char *output_ptr, int *output_len, const char *alphabet_id_ptr,
                           const char *input_ptr, const char *key_ptr, int key_len,
                           const char *tweak_ptr, int tweak_len,
                           const char *additional_characters_ptr);
 ...
 * memory.
 */
int h_fpe_encrypt_float(double *output, double input, const char *key_ptr, int key_len,
                        const char *tweak_ptr, int tweak_len);
 ...
 * memory.
 */
int h_fpe_decrypt_float(double *output, double input, const char *key_ptr, int key_len,
                        const char *tweak_ptr, int tweak_len);
 ...
 * error.
 */
int h_fpe_encrypt_integer(unsigned long long *output, unsigned long long input, unsigned int radix,
                          unsigned int digits, const char *key_ptr, int key_len,
                          const char *tweak_ptr, int tweak_len);
 ...
 * error.
 */
int h_fpe_decrypt_integer(unsigned long long *output, unsigned long long input, unsigned int radix,
                          unsigned int digits, const char *key_ptr, int key_len,
                          const char *tweak_ptr, int tweak_len);
 ...
 */
int h_fpe_encrypt_big_integer(unsigned char *output_ptr, int *output_len, const char *input_ptr,
                              unsigned int radix, unsigned int digits, const char *key_ptr,
                              int key_len, const char *tweak_ptr, int tweak_len);
 ...
 * Returns 0 on success, -1 on error.
 */
int h_fpe_decrypt_big_integer(unsigned char *output_ptr, int *output_len, const char *input_ptr,
                              unsigned int radix, unsigned int digits, const char *key_ptr,
                              int key_len, const char *tweak_ptr, int tweak_len);
